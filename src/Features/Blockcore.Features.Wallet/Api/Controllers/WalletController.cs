...
namespace Blockcore.Features.Wallet.Api.Controllers
{
    ...
    public class WalletController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        public IActionResult GetPublicKey(string address, string walletName, string accountName)
        {
            ...
            Types.Wallet aliceWallet = this.walletManager.GetWallet(walletName);
            HdAddress AliceHdAddress = aliceWallet.GetAddress(address, account => account.Name.Equals(accountName));

            try
            {
                return Ok(AliceHdAddress.Pubkey.ToHex());
